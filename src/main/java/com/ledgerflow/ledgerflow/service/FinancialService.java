...
public class FinancialService {
    ...
    public FinancialMetricsDto calculateMetrics(User user, LocalDate startDate, LocalDate endDate) {
        ...
        // Use date-range queries so the dashboard period filter actually works
        BigDecimal fixedExpenses = expenseService.getTotalExpensesByTypeAndDateRange(user, "FIXED", startDate, endDate);
        BigDecimal variableExpenses = expenseService.getTotalExpensesByTypeAndDateRange(user, "VARIABLE", startDate, endDate);
        BigDecimal totalIncome = incomeService.getIncomeForDateRange(user, startDate, endDate);

        metrics.setTotalFixedExpenses(fixedExpenses);
        metrics.setTotalVariableExpenses(variableExpenses);
        metrics.setTotalExpenses(fixedExpenses.add(variableExpenses));
        metrics.setTotalIncome(totalIncome);

        BigDecimal netProfit = totalIncome.subtract(metrics.getTotalExpenses());
        metrics.setNetProfit(netProfit);
        metrics.setSavings(netProfit);

        if (totalIncome.compareTo(BigDecimal.ZERO) > 0) {
            double savingsRate = netProfit.doubleValue() / totalIncome.doubleValue() * 100;
            metrics.setSavingsRate(savingsRate);
        } else {
            metrics.setSavingsRate(0.0);
        }

        // Category breakdowns filtered by date range
        List<Object[]> categoryExpensesList = expenseService.getCategoryWiseExpensesForDateRange(user, startDate, endDate);
